...
export function CashbackSection({
  ...
  const sharePercent = useWatch({
    control: form.control,
    name: "cashback_share_percent",
  });
  const shareFixed = useWatch({
    control: form.control,
  ...
  const [cycleStats, setCycleStats] = useState<AccountSpendingStats | null>(
    null,
  );
  ...
  const policy = useMemo(() => {
    ...
    const cycleSpent =
      cycleStats?.currentSpend || activeAccount.stats?.spent_this_cycle || 0;
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <p className="text-[10px] text-slate-400 font-medium whitespace-nowrap">
                  {cycleStats?.cycle?.label ||
                  activeAccount?.credit_card_info?.statement_day
                    ? `Cycle: ${
                        cycleStats?.cycle?.label ||
                        `resets on ${activeAccount?.credit_card_info?.statement_day}`
                      }`
                    : "Checking cycle..."}
                </p>
              </div>
              ...
              render={({ field }) => (
                <FormItem className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <FormLabel className="text-[10px] font-black text-slate-400 uppercase tracking-wider">
                        Rate (%)
                      </FormLabel>
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      {(field.value || 0) > 100 && (
                        <div className="flex items-center gap-1.5 text-[9px] font-bold text-rose-500 px-1">
                          ...
                          Efficiency &gt; 100% means sharing more than total
                          amount!
                        </div>
                      )}
                    </div>
                ...
                <FormItem className="space-y-1.5 focus-within:z-10">
                  ...
                  <FormControl>
                    <SmartAmountInput
                      ...
                      onChange={(val) =>
                        field.onChange(
                          val !== undefined ? Math.max(0, val) : null,
                        )
                      }
                      hideLabel={true}
                      placeholder="Amount"
                  ...
                  >
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="bg-white/10 p-2 rounded text-[11px] font-mono">
                        ...
                        <div className="mt-1 pt-1 border-t border-white/10 text-amber-400 font-bold">
                          = {formatVN(totalSharedVal)}
                        </div>
                      </div>
                ...
                <div className="pt-4 mt-2 border-t border-dashed border-slate-200">
                    ...
                    <div className="bg-white border border-slate-100 rounded-lg p-2 shadow-sm">
                      {(() => {
                        ...
                        const activeRule = ruleId
                          ? cycleStats?.activeRules?.find(
                              (r) => r.ruleId === ruleId,
                            )
                          : null;
                        const rawRuleRemaining = activeRule
                          ? activeRule.max! - activeRule.earned
                          : (policy?.maxReward ??
                            cycleStats?.remainingBudget ??
                            remainsCap ??
                            null);
                        const ruleRemaining =
                          rawRuleRemaining === null
                            ? null
                            : Math.max(0, rawRuleRemaining - actualBankReward);
                          ...
                          <>
                            ...
                            <span
                              className={cn(
                                ...
                                ruleRemaining === null || ruleRemaining > 0
                                  ? "text-indigo-600"
                                  : "text-rose-600",
                              )}
                            >
                              {ruleRemaining !== null
                                ? formatVN(ruleRemaining)
                                : "Unlimited"}
                            </span>
                          </>
                        );
                  ...
                  {(() => {
                    ...
                    const activeRule = ruleId
                      ...
                      : null;
                    const rawRewardRemaining = activeRule
                      ? activeRule.max! - activeRule.earned
                      : (policy?.maxReward ??
                        cycleStats?.remainingBudget ??
                        remainsCap ??
                    ...
                    const rewardRemaining =
                      rawRewardRemaining === null
                        ? null
                        : Math.max(0, rawRewardRemaining - actualBankReward);

                    if (
                      rewardRemaining === null ||
                      rewardRemaining === Infinity ||
                      !policy?.rate ||
                      policy.rate <= 0
                    )
                      return null;
                      ...
                      >
                        ...
                        {isOverbudget && (
                          <p className="text-[7px] text-rose-500 font-bold italic mt-1 uppercase tracking-tight">
                            Note: amount &gt; remains means NO cashback earned
                            for overflow.
                          </p>
                        )}
                      </div>
