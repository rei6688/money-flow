...
function stripFeeMarkers(note: string): string {
  return (note || "")
    .replace(/\s*#Fee=[\d.,]+\s*/g, " ")
    .replace(/\s*\(\s*[\d.,]+\s*\|\s*Fee:\s*[\d.,]+\s*\)\s*/g, " ")
    .replace(/\s{2,}/g, " ")
    .trim();
}

function upsertFeeNote(note: string, principal: number, fee: number): string {
...
export function TransactionSlideV2({
  ...
  const defaultFormValues = useMemo((): SingleTransactionFormValues => {
    ...
    if (initialData) {
      ...
      let type: SingleTransactionFormValues["type"] =
      ...
      const values: SingleTransactionFormValues = {
        ...
        source_account_id: isIncome
          ? null
          : (initialData.source_account_id ?? null),
    ...
    return async (values: any, context: any, options: any) => {
      try {
        ...
        if (error instanceof TypeError && error.message.includes("_zod")) {
          console.error(
            "⚠️ This looks like a Zod/HookForm version mismatch or malformed schema issue.",
          );
        }
  ...
  useEffect(() => {
    if (mode === "bulk") {
      ...
      const hasBulkChanges =
        ...
        bulkRows &&
        bulkRows.length > 0 &&
        bulkRows.some((row: any) => row.amount > 0 || row.note || row.shop_id);
  ...
  useEffect(() => {
    ...
    const subscription = singleForm.watch((currentValues) => {
      ...
      const hasActualChanges =
        ...
        currentValues.source_account_id !==
          defaultFormValues.source_account_id ||
        currentValues.target_account_id !==
          defaultFormValues.target_account_id ||
        ...
        currentValues.cashback_share_percent !==
  ...
  const sourceAccId = useWatch({
    control: singleForm.control,
    name: "source_account_id",
  });
  const targetAccId = useWatch({
    control: singleForm.control,
    name: "target_account_id",
  });
  const currentTxnType = useWatch({
    control: singleForm.control,
    name: "type",
  });
  const currentPersonId = useWatch({
    control: singleForm.control,
    name: "person_id",
  });
  ...
  useEffect(() => {
    ...
    if (
      currentPersonId &&
      (currentTxnType === "transfer" || currentTxnType === "credit_pay")
    ) {
  ...
  useEffect(() => {
    ...
    if (prevTypeRef.current !== type) {
      ...
      // 2. Logic for switching FROM Special Modes back to regular modes (Expense/Debt/Income/Repayment)
        ...
        ["credit_pay", "transfer"].includes(prevTypeRef.current) &&
        ["expense", "debt", "income", "repayment"].includes(type)
      ) {
        // If we had a CC in target, move it back to source for Expense/Debt
        if (
          currentTargetAcc?.type === "credit_card" &&
  ...
  useEffect(() => {
    ...
    if (!open || operationMode === "edit" || operationMode === "duplicate")
      return;
  ...
  const onSingleSubmit = async (data: SingleTransactionFormValues) => {
    ...
    const payload: any = {
      ...
      target_account_id:
        data.type === "transfer" || data.type === "credit_pay"
      ...
      cashback_share_percent: data.cashback_share_percent
        ? data.cashback_share_percent / 100
        : null,
        ...
        toast.error(
          <div className="flex flex-col gap-1">
            ...
            <span className="text-xs">
              Không được sửa tại đây để tránh lệch Data.
            </span>
            {batchId && (
              <a
            ...
            {isLoadingEdit && (
              <div className="absolute inset-0 z-50 bg-white/60 backdrop-blur-[1px] flex items-center justify-center animate-in fade-in duration-200">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="text-sm font-bold text-slate-900 uppercase tracking-widest animate-pulse">
                    Loading Details...
                  </span>
                </div>
              </div>
