...
export const UnifiedTransactionTable = React.forwardRef<
  UnifiedTransactionTableRef,
...
>(
  (
    ...
    const [tableData, setTableData] = useState<TransactionWithDetails[]>(
      () => data ?? transactions ?? [],
    );
    const [updatingTxnIds, setUpdatingTxnIds] = useState<Set<string>>(
      new Set(),
    );
    ...
    const optimisticTxns = useRef<Map<string, TransactionWithDetails>>(
      new Map(),
    );
    ...
    const handleOptimisticUpdate = useCallback(
      (optimisticTxn: TransactionWithDetails) => {
        // Safety check: if no ID, cannot update
        if (!optimisticTxn?.id) {
          console.warn(
            "[Optimistic Update] Transaction has no ID, skipping update",
          );
          return;
        }
    ...
    const mobileColumnOrder: ColumnKey[] = [
      "date",
      "shop",
      "category",
      "account",
    ...
    useEffect(() => {
      if (customColumnOrder.length > 0)
        localStorage.setItem(
          "mf_v3_col_order",
          JSON.stringify(customColumnOrder),
        );
    }, [customColumnOrder]);
    ...
    const [selectionStartId, setSelectionStartId] = useState<string | null>(
      null,
    );
    ...
    const selectedStats = useMemo(() => {
      if (selectedCells.size === 0 || !selectedColumn)
        return { totalIn: 0, totalOut: 0, average: 0, count: 0 };
      ...
      selectedCells.forEach((id) => {
        ...
        if (txn) {
          ...
          if (selectedColumn === "amount") {
            ...
            const hasCashback =
              percentRaw > 0 || fixedRaw > 0 || shareAmount > 0;
            ...
            if (hasCashback) {
              ...
              if (["expense", "debt", "transfer"].includes(txn.type))
                val = -finalDisp;
              else if (["income", "repayment"].includes(txn.type))
                val = finalDisp;
    ...
    const [editingTxn, setEditingTxn] = useState<TransactionWithDetails | null>(
      null,
    );
    ...
    const [historyTarget, setHistoryTarget] =
      useState<TransactionWithDetails | null>(null);
    const [confirmDeletingTarget, setConfirmDeletingTarget] =
      useState<TransactionWithDetails | null>(null);
    ...
    const [statusOverrides, setStatusOverrides] = useState<
      Record<string, TransactionWithDetails["status"]>
    >({});
    const [refundFormTxn, setRefundFormTxn] =
      useState<TransactionWithDetails | null>(null);
          ...
          toast.error(
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs">
                Không được xóa tại đây để tránh lệch Data.
              </span>
              {batchId && (
                <a
      ...
      const filtered = tableData.filter((txn) => {
        ...
        if (searchQuery) {
          ...
          const match =
            ...
            (
              (txn.metadata as any)?.original_description?.toLowerCase() || ""
            ).includes(q);
    ...
    const handleSelectOne = (
      ...
      if (shiftKey && lastSelectedIdRef.current) {
        const startIdx = displayedTransactions.findIndex(
    ...
    const inferTieredPolicyByCategoryName = useCallback(
      (account: Account | undefined, categoryName: string | undefined) => {
        if (
          !account ||
          account.type !== "credit_card" ||
          account.cb_type !== "tiered" ||
          !categoryName
        )
          return null;
        const tiers = Array.isArray((account.cb_rules_json as any)?.tiers)
          ? (account.cb_rules_json as any).tiers
          : [];
        const policies = (tiers[0]?.policies || []) as Array<{
          rate?: number;
          max?: number;
        }>;
        ...
        const byRateAsc = [...normalizedPolicies].sort(
          (left, right) => left.rate - right.rate,
        );
        const byRateDesc = [...normalizedPolicies].sort(
    ...
    const estimateTxnCashback = useCallback(
      (txn: TransactionWithDetails) => {
        const scopedAccount =
          context === "account" && contextId
        ...
        if (!account || account.type !== "credit_card")
          return {
            estimated: 0,
            rate: 0,
            maxReward: undefined as number | undefined,
            isFallback: false,
          };
        ...
        const shouldUseFallback =
          Boolean(fallback) &&
          (policy.metadata?.policySource === "program_default" ||
            policy.metadata?.policySource === "level_default");
        const effectiveRate = shouldUseFallback
          ? Number(fallback?.rate || policy.rate || 0)
          : Number(policy.rate || 0);
        const effectiveMaxReward = shouldUseFallback
          ? fallback?.maxReward
          : policy.maxReward;

        const baseVal = amountAbs * effectiveRate;
        const estimated =
          effectiveMaxReward !== undefined && effectiveMaxReward !== null
    ...
    const displayedColumns = useMemo(() => {
      if (isMobile) {
        return mobileColumnOrder
          ...
          .filter(Boolean)
          .filter((col) => visibleColumns[col!.key]) as ColumnConfig[];
      }

      return effectiveColumnOrder
        .map((key) => defaultColumns.find((col) => col.key === key))
        .filter(
          (col): col is ColumnConfig =>
            !!col &&
    ...
    const isAllSelected =
      ...
      selection.size >= displayedTransactions.length;
      ...
      <div className="relative flex flex-col w-full h-full min-h-0">
        <div
          className={cn(
            ...
            isExcelMode &&
              "border-emerald-500 shadow-emerald-100 ring-4 ring-emerald-50",
          )}
              ...
              renderActions={
                isMobile
                  ? (txn) =>
                      renderRowActions(
                        txn,
                        (statusOverrides[txn.id] ?? txn.status) === "void",
                      )
                  : undefined
              }
              onRowClick={(txn) => {
                ...
                <TableHeader className="sticky top-0 z-30 bg-gradient-to-b from-slate-50 to-white backdrop-blur-sm border-b-2 border-slate-200 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.1)]">
                  <TableRow className="hover:bg-transparent border-0">
                    {displayedColumns.map((col) => {
                      ...
                      const isMobileCategoryDate =
                        isMobile && col.key === "category";
                      const columnLabel = isMobileCategoryDate
                        ? "Category / Date"
                        : col.label;

                      // Check if any sort is active
                      const isSorted =
                        sortState.key !== "date" || sortState.dir !== "desc";
                  ...
                  {paginatedTransactions.length === 0 ? (
                    ...
                    paginatedTransactions.map((txn, rowIndex) => {
                      ...
                      const txnDestId =
                        ...
                        (txn as any).target_account_id ||
                        (destNameRaw !== "Unknown"
                          ? accounts.find((a) => a.name === destNameRaw)?.id
                          : undefined);
                      ...
                      if (
                        ...
                        txnMetadata?.original_transaction_id &&
                        !txnMetadata.is_refund_confirmation
                      )
                        refundSeq = 2;
                      else if (txnMetadata?.is_refund_confirmation)
                        refundSeq = 3;

                      let displayIdForBadge = txn.id;
                      if (refundSeq === 2 || refundSeq === 3) {
                        displayIdForBadge =
                          (txnMetadata?.original_transaction_id as string) ||
                          txn.id;
                      }
                      ...
                      if (isVoided) {
                        rowBgColor =
                          "opacity-80 bg-gray-50 scale-[0.99] border-dashed";
                      } else {
                        const refundSeqCheck =
                          (txn.metadata as any)?.refund_sequence || 0;
                        if (txn.is_installment || txn.installment_plan_id)
                          rowBgColor = "bg-amber-50";
                        else if (refundSeqCheck > 0)
                          rowBgColor = "bg-purple-50"; // Refund shading
                        else if (txn.type === "repayment")
                          rowBgColor = "bg-slate-50";
                        else if (
                          effectiveStatus === "pending" ||
                          effectiveStatus === "waiting_refund"
                        )
                          rowBgColor = "bg-emerald-50/50";
                      }

                      const renderCell = (key: ColumnKey) => {
                        const refundAccount = accounts.find(
                          (a) => a.id === txn.account_id,
                        ); // Lifted up for Category case availability
                        switch (key) {
                          case "date": {
                            const d = new Date(
                              txn.occurred_at ?? txn.created_at ?? Date.now(),
                            );
                            const day = String(d.getDate()).padStart(2, "0");
                          ...
                          case "actual_cashback": {
                            ...
                            const effectiveRate =
                              amountAbs > 0 ? val / amountAbs : 0;
                            const isCapped =
                              estimate.maxReward !== undefined &&
                              estimate.maxReward !== null &&
                              baseVal > estimate.maxReward;
                                ...
                                content={
                                  <div className="text-xs space-y-1.5">
                                    ...
                                    <div className="text-slate-400">
                                      ...
                                      {(policyRate * 100).toFixed(
                                        (policyRate * 100) % 1 === 0 ? 0 : 2,
                                      )}
                          ...
                          case "est_share": {
                            ...
                            const { fixedRaw, percentRaw, shareAmount } =
                              resolveCashbackFields(txn);
                            const shareRate =
                              percentRaw > 1 ? percentRaw / 100 : percentRaw;
                            const amountAbs = Math.abs(txn.amount);
                            const computedShared =
                              amountAbs * shareRate + fixedRaw;
                            const val =
                            ...
                            if (shareRate > 0) {
                              const percentLabel = (shareRate * 100).toFixed(
                                (shareRate * 100) % 1 === 0 ? 0 : 2,
                              );
                              formulaLabelParts.push(`${percentLabel}%`);
                            }
                            const formulaLabel =
                              formulaLabelParts.length > 0
                                ? formulaLabelParts.join(" + ")
                                : numberFormatter.format(val);
                                  ...
                                  <div className="text-xs space-y-1">
                                    ...
                                    <div className="text-slate-400">
                                      {shareRate > 0 && (
                                        <span>
                                          {numberFormatter.format(amountAbs)} ×{" "}
                                          {(shareRate * 100).toFixed(
                                            (shareRate * 100) % 1 === 0 ? 0 : 2,
                                          )}
                                          %
                                        </span>
                                      )}
                                      {shareRate > 0 && fixedRaw > 0 && (
                                        <span> + </span>
                                      )}
                                      {fixedRaw > 0 && (
                                        <span>
                          ...
                          case "net_profit": {
                            ...
                            const estimateBack = Math.max(
                              Number(estimate.estimated || 0),
                              bankBack,
                            );
                                ...
                                content={
                                  <div className="text-xs space-y-1">
                                    ...
                                    <div className="text-slate-400">
                                      ...
                                      (est) - {numberFormatter.format(share)}{" "}
                                      (share) ={" "}
                                      <span
                                        className={
                                          profit > 0
                          ...
                          case "shop": {
                            const resolvedShop = txn.shop_id
                              ? shops.find((shop) => shop.id === txn.shop_id)
                              : null;
                            let shopLogo =
                              txn.shop_image_url ||
                              resolvedShop?.image_url ||
                              null;

                            // ROLLOVER IMAGE OVERRIDE: If category is Rollover, use Category Image (takes precedence over Shop/Bank)
                            if (
                              txn.category_name === "Rollover" ||
                              txn.category_id ===
                                "71e71711-83e5-47ba-8ff5-85590f45a70c"
                            ) {
                              const rolloverCat = categories.find(
                                (c) =>
                                  c.id ===
                                  "71e71711-83e5-47ba-8ff5-85590f45a70c",
                              );
                            ...
                            if (!shopLogo) {
                              if (
                                ...
                                const destId =
                                  (txn as any).destination_account_id ||
                                  (txn as any).target_account_id;
                            ...
                            if (
                              ...
                              const repaymentAccount = txnSourceId
                                ? accounts.find(
                                    (account) => account.id === txnSourceId,
                                  )
                                : null;
                              const repaymentLogo =
                                txn.source_image ??
                                repaymentAccount?.image_url ??
                                null;
                              shopLogo = repaymentLogo;
                            }

                            const isServicePayment =
                              txn.note?.startsWith("Payment for Service") ||
                              (txn.metadata as any)?.type === "service_payment";
                            ...
                            const accountName =
                              (txn.accounts as any)?.name ||
                              refundAccount?.name;
                            const isPendingRefund =
                              txn.account_id === REFUND_PENDING_ACCOUNT_ID ||
                              accountName === "Pending Refunds (System)";
                            ...
                            const isOriginalTxn = Boolean(
                              originalMetadata.original_transaction_id,
                            );
                            const isRefundConfirmation = Boolean(
                              originalMetadata.is_refund_confirmation,
                            );
                            const isRefundRequest =
                              isOriginalTxn && !isRefundConfirmation;

                            const canConfirm =
                              isRefundRequest && txn.status !== "completed";
                              ...
                              <div className="flex items-center gap-2 w-full overflow-hidden group">
                                ...
                                <div className="flex items-center gap-2 min-w-0 flex-1 justify-between">
                                  ...
                                  <div className="flex items-center gap-1.5 min-w-0 flex-1">
                                    ...
                                    <CustomTooltip content="Open DB (new tab)">
                                      <button
                                        onClick={(e) => {
                                          e.stopPropagation();
                                    ...
                                    {(() => {
                                      ...
                                      if (!note) {
                                        return (
                                          <span className="text-slate-400 italic text-[0.9em]">
                                            No note
                                          </span>
                                        );
                                      }
                                  ...
                                  {(() => {
                                    ...
                                    const isSplitParent =
                                      ...
                                      metadata?.is_split_bill_base === true;
                                    const isSplitChild = !!(
                                      metadata?.parent_transaction_id ||
                                      metadata?.split_parent_id
                                    );
                                    const splitGroupName =
                                      metadata?.split_group_name;

                                    let splitBadge = null;
                                    if (isSplitParent || isSplitChild) {
                                      const badgeText = isSplitParent
                                        ? "SPLIT"
                                        : "SHARE";
                                      ...
                                      const tooltipText = isSplitParent
                                        ? splitGroupName
                                          ? `Split Bill Parent - Group: ${splitGroupName}`
                                          : "Split Bill Parent (Total)"
                                    ...
                                    const duplicatedFromId =
                                      metadata?.duplicated_from_id;
                                    ...
                                    const hasBulkDebts =
                                      metadata?.bulk_allocation?.debts?.length >
                                        0 ||
                                      metadata?.bulkAllocation?.debts?.length >
                                        0;
                                    const currentInstallmentBadge =
                                      txn.is_installment ||
                                        ...
                                        {hasBulkDebts &&
                                          (() => {
                                            const bulkAllocation =
                                              metadata?.bulk_allocation ||
                                              metadata?.bulkAllocation;

                                            if (
                                              bulkAllocation?.debts &&
                                              bulkAllocation.debts.length > 0
                                            ) {
                                              const debts =
                                                bulkAllocation.debts as {
                                                  id: string;
                                                  amount: number;
                                                  tag?: string;
                                                  note?: string;
                                              ...
                                              return (
                                                <CustomTooltip
                                                  content={
                                                    <div className="flex flex-col gap-1">
                                                      <span className="font-semibold border-b border-slate-600 pb-1 mb-1">
                                                        Repayment for {count}{" "}
                                                        items:
                                                      </span>
                                                      {debts.map((d, i) => (
                          ...
                          case "note": {
                            ...
                            return (
                              <div className="flex items-center gap-2 max-w-none group/note justify-between w-full min-w-0">
                                ...
                                <div className="flex items-center gap-1.5 min-w-0 flex-1">
                                  {linkedIdForCopy &&
                                    linkedIdForCopy !== txn.id && (
                                      <button
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          copyToClipboard(linkedIdForCopy).then(
                                            (ok) => {
                                              if (!ok) return;
                          ...
                          case "category": {
                            ...
                            const categoryImage =
                              (actualCategory as any)?.image_url ||
                              actualCategory?.image_url ||
                              txn.category_image_url ||
                              null;
                            const categoryIcon =
                              (actualCategory as any)?.icon ||
                              txn.category_icon ||
                              null;

                            // Internal vs External Logic
                            const isInternal =
                              actualCategory?.kind === "internal";
                          ...
                          case "account": {
                            ...
                            const sourceAccountFromId = sourceId
                              ? accounts.find((a) => a.id === sourceId)
                              : null;
                            const sourceName =
                              txn.source_name ||
                              txn.account_name ||
                              sourceAccountFromId?.name ||
                              "Unknown";
                            const sourceIcon =
                              txn.source_image ||
                            ...
                            const resolvedPerson = personId
                              ...
                              : null;
                            const personName =
                              (txn as any).person_name ||
                              resolvedPerson?.name ||
                              "Unknown";
                            const personImage =
                              (txn as any).person_image_url ||
                              resolvedPerson?.image_url ||
                              null;

                            const destId = txnDestId;
                            const destAccountFromId = destId
                              ? accounts.find((a) => a.id === destId)
                              : null;
                            const destName =
                              txn.destination_name ||
                              destAccountFromId?.name ||
                              destNameRaw;
                            const destIcon =
                              txn.destination_image ||
                              destAccountFromId?.image_url ||
                              null;
                            ...
                            const cycleTag =
                              normalizeMonthTag(
                                (txn as any).persisted_cycle_tag,
                              ) ?? (txn as any).persisted_cycle_tag;
                            const debtTag = personId
                            ...
                            if (txn.type === "expense") {
                              ...
                              typeIcon = (
                                <CustomTooltip content="Debt">
                                  <UserMinus className="h-4 w-4 text-amber-600" />
                                </CustomTooltip>
                              );
                            } else if (txn.type === "repayment") {
                              typeIcon = (
                                <CustomTooltip content="Repayment">
                            ...
                            const sourceCycleBadge =
                              sourceAccount &&
                              sourceAccount.type === "credit_card" &&
                              sourceAccount.cashback_config ? (
                                <CycleBadge
                            ...
                            const cycleSheet = person?.cycle_sheets?.find(
                              (s) => s.cycle_tag === debtTag,
                            );
                            const sheetUrl =
                              cycleSheet?.sheet_url ||
                                  ...
                                  >
                                    <span
                                      onClick={(e) => {
                                        ...
                                        e.preventDefault();
                                        window.open(
                                          `/people/${personRouteId}?tag=${debtTag}`,
                                          "_blank",
                                          "noopener,noreferrer",
                                        );
                                      }}
                            ...
                            const showSingleFlow =
                              (!hasTarget && !hasPerson) ||
                              isPersonContext ||
                              (hasPerson && isSourceContext) ||
                              (hasTarget && isDestContext) ||
                              (hasTarget && isSourceContext);
                            ...
                            if (showSingleFlow) {
                              ...
                              if (entityToShow === "person") {
                                ...
                                if (
                                  ...
                                  displayName =
                                    cat?.name || txn.category_name || "General";
                            ...
                            const sourceBadges = [sourceCycleBadge].filter(
                              Boolean,
                            );

                            // Target badges - debt tag for people OR nothing for accounts
                            const targetBadges: React.ReactNode[] = hasPerson
                            ...
                            return (
                              <div className="flex flex-wrap gap-1 min-w-[120px] justify-end">
                                ...
                                {!txn.tag &&
                                  !txn.is_installment &&
                                  !txn.installment_plan_id && (
                                    <span className="text-slate-400 opacity-50 text-xs">
                                      -
                                    </span>
                                  )}
                              </div>
                          ...
                          case "amount": {
                            ...
                            const rate =
                              percentDisp > 1 ? percentDisp / 100 : percentDisp;
                      ...
                      return (
                        <TableRow
                          ...
                          className={cn(
                            ...
                            (updatingTxnIds.has(txn.id) ||
                              loadingIds?.has(txn.id)) &&
                              "opacity-70 animate-pulse bg-slate-50",
                            successTxnIds.has(txn.id) &&
                              "bg-emerald-50/10 shadow-[inset_0_0_20px_rgba(16,185,129,0.05)]",
                          )}
                        >
                          ...
                          {displayedColumns.map((col) => {
                            ...
                            return (
                              <TableCell
                                key={`${txn.id}-${col.key}`}
                                onMouseDown={(e) =>
                                  handleCellMouseDown(txn.id, col.key, e)
                                }
                                onMouseEnter={() =>
                                ...
                                className={cn(
                                  ...
                                  isExcelMode &&
                                    "select-none cursor-crosshair active:cursor-crosshair",
                                  isExcelMode &&
                                    selectedCells.has(txn.id) &&
                                    col.key === "amount" &&
                  ...
                  <tfoot className="sticky bottom-0 z-40 bg-slate-900 text-white font-black shadow-[0_-4px_12px_rgba(0,0,0,0.2)] border-t-2 border-slate-700">
                    <TableRow className="hover:bg-slate-900 border-0">
                      {displayedColumns.map((col, idx) => {
                        ...
                        if (isFirst) {
                          content = (
                            <span className="text-[10px] uppercase tracking-widest opacity-60 ml-8">
                              Total Rows
                            </span>
                          );
                  ...
                  <div className="fixed bottom-0 left-0 right-0 flex md:hidden bg-white border-t border-slate-200 px-3 py-2 items-center justify-between gap-2 z-40 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        onClick={() =>
                          setCurrentPage(
                            Math.min(calculatedTotalPages, currentPage + 1),
                          )
                        }
              ...
              >
                ...
                >
                  ...
                  <p className="mt-2 text-sm text-slate-600">
                    ...
                    <span className="font-bold text-rose-600">VOID</span>. It
                  ...
                  {(confirmVoidTarget.note?.includes("[C]") ||
                    (typeof confirmVoidTarget.metadata === "string" &&
                      confirmVoidTarget.metadata?.includes("batch_id")) ||
                    (confirmVoidTarget.metadata as any)?.batch_id) && (
                    <div className="mt-3 p-3 bg-amber-50 border border-amber-100 rounded-lg flex gap-3">
                      ...
                      <div className="text-[11px] font-medium text-amber-800 leading-normal">
                        <strong className="block text-amber-900 mb-0.5 uppercase tracking-wider">
                          Confirmed Transaction Detected
                        </strong>
                        This transaction is part of a BATCH. Voiding it here
                        will automatically UNCHECK (revert) the corresponding
                        item in the Batch Checklist.
                      </div>
                    </div>
                  )}
                  {voidError && (
              ...
              >
                ...
                >
                  ...
                  <p className="mt-2 text-xs text-amber-600">
                    Money will stay in &quot;Pending&quot; account until you
                    confirm receipt.
                  </p>
                  {voidError && (
            ...
            createPortal(
              ...
              >
                <div
                  ...
                  onClick={(event) => event.stopPropagation()}
                >
                  <h3 className="text-lg font-semibold text-slate-900 text-rose-600">
                    Delete Forever?
                  </h3>
                  <p className="mt-2 text-sm text-slate-600">
                    This will{" "}
                    ...
                    <span className="font-bold italic">CANNOT be undone</span>{" "}
            ...
            (() => {
              const baseAmount =
                refundFormStage === "confirm"
                  ...
                  : Math.abs(
                      refundFormTxn.original_amount ??
                        refundFormTxn.amount ??
                        0,
                    );
              ...
              const defaultAccountId =
                (refundFormStage === "confirm"
                  ? null
                  : refundFormTxn.account_id) ??
                refundAccountOptions[0]?.id ??
                null;
              const initialNote =
                refundFormStage === "confirm"
                  ...
                  >
                    ...
                    <TransactionForm
                      ...
                      initialValues={{
                        ...
                        occurred_at: refundFormTxn.occurred_at
                          ? new Date(refundFormTxn.occurred_at)
                          : new Date(),
              ...
              >
                ...
                >
                  ...
                  <div className="mt-4 flex justify-end gap-2">
                    ...
                    <button
                      className={`inline-flex items-center justify-center rounded-md px-3 py-1 text-sm font-semibold text-white shadow-sm disabled:cursor-not-allowed disabled:opacity-70 ${
                        bulkDialog.mode === "restore"
