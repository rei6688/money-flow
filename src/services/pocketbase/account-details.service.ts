...
function parseCycleTagFromTransaction(record: PocketBaseRecord): string | null {
  ...
  if (
    ...
    record.metadata.persisted_cycle_tag
  ) {
...
function inferTieredPolicyByCategoryName(
  account: Account,
  categoryName: string | undefined,
): { rate: number; maxReward?: number } | null {
...
function mapTransaction(
  ...
  const sourceAccountPocketBaseId =
    expandedAccount?.id || record.account_id || null;
  const targetAccountPocketBaseId =
    expandedTargetAccount?.id || record.to_account_id || null;
  const sourceAccountSourceId =
    expandedAccount?.slug ||
    (record.account_id === toPocketBaseId(currentAccountSourceId, "accounts")
      ? currentAccountSourceId
      : record.account_id);
  const targetAccountSourceId =
    expandedTargetAccount?.slug || record.to_account_id || null;
  ...
  return {
    ...
    cashback_share_percent:
      ...
      record.metadata?.cashback_share_percent ??
      null,
    cashback_share_fixed:
      record.cashback_share_fixed ??
      record.metadata?.cashback_share_fixed ??
      null,
...
export async function deletePocketBaseCategory(
  supabaseId: string,
): Promise<boolean> {
  const pbId = toPocketBaseId(supabaseId);
...
export async function deletePocketBaseCategoriesBulk(
  supabaseIds: string[],
): Promise<boolean> {
...
export async function createPocketBaseShop(
  ...
  const pbId = toPocketBaseId(supabaseId);
  const pbCategoryId = data.default_category_id
    ? toPocketBaseId(data.default_category_id)
    : null;
...
export async function updatePocketBaseShop(
  ...
  if (typeof data.default_category_id !== "undefined") {
    body.default_category_id = data.default_category_id
      ? toPocketBaseId(data.default_category_id)
      : null;
  }
  ...
  try {
    await pocketbaseRequest<Record<string, unknown>>(
      `/api/collections/shops/records/${pbId}`,
      {
        method: "PATCH",
        body,
      },
...
export async function deletePocketBaseShop(
  supabaseId: string,
): Promise<boolean> {
  const pbId = toPocketBaseId(supabaseId);
...
export async function togglePocketBaseShopsArchiveBulk(
  ...
  const results = await Promise.allSettled(
    supabaseIds.map((sbId) =>
      pocketbaseRequest(
        `/api/collections/shops/records/${toPocketBaseId(sbId)}`,
        {
          method: "PATCH",
          body: { is_archived: isArchived },
        },
...
export async function deletePocketBaseShopsBulk(
  supabaseIds: string[],
): Promise<boolean> {
...
export async function createPocketBasePerson(
  ...
  try {
    await pocketbaseRequest<Record<string, unknown>>(
      "/api/collections/people/records",
      {
        method: "POST",
        body: {
          id: pbId,
          ...
          group_parent_id: data.group_parent_id
            ? toPocketBaseId(data.group_parent_id)
            : null,
...
export async function updatePocketBasePerson(
  ...
  try {
    await pocketbaseRequest<Record<string, unknown>>(
      `/api/collections/people/records/${pbId}`,
      {
        method: "PATCH",
        body,
      },
...
export async function createPocketBaseAccount(
  ...
  const pbParentId = data.parent_account_id
    ...
    : null;
  const pbSecuredById = data.secured_by_account_id
    ? toPocketBaseId(data.secured_by_account_id)
    : null;
  const pbHolderPersonId = data.holder_person_id
    ? toPocketBaseId(data.holder_person_id)
    : null;
...
export async function updatePocketBaseAccountInfo(
  ...
  if ("secured_by_account_id" in body && body.secured_by_account_id) {
    body.secured_by_account_id = toPocketBaseId(
      body.secured_by_account_id as string,
    );
...
export async function getPocketBaseAccountSpendingStatsSnapshot(
  ...
  const cycleResponse = await pocketbaseList<PocketBaseRecord>(
    "cashback_cycles",
    {
      perPage: 1,
  ...
  for (let attemptIdx = 0; attemptIdx < queryAttempts.length; attemptIdx++) {
    ...
    try {
      ...
      console.warn(
        "[DB:PB] account spending stats: transaction query attempt failed",
        {
          sourceAccountId,
  ...
  if (rawTransactions.length === 0) {
    console.warn(
      "[DB:PB] account spending stats: all transaction query attempts exhausted, falling back to cycle snapshot",
      {
        sourceAccountId,
        cycleTag: resolvedCycleTag,
      },
  ...
  const cycleTransactions = rawTransactions.filter((tx) => {
    ...
    return (
      txDate.getTime() >= cycleStartTime && txDate.getTime() <= cycleEndTime
    );
  });

  const categoryIds = Array.from(
    new Set(cycleTransactions.map((tx) => tx.category_id).filter(Boolean)),
  );
  ...
  if (categoryIds.length > 0) {
    const categoryResponse = await pocketbaseList<PocketBaseRecord>(
      "categories",
      {
        perPage: 200,
  ...
  if (cycleTransactions.length === 0) {
    estimatedCashback =
      Number(cycle?.virtual_profit ?? 0) + Number(cycle?.real_awarded ?? 0);
    ...
    actualClaimed = cycleTransactions.reduce((sum, tx) => {
      ...
      const isCashbackIncome =
        categoryName.toLowerCase().includes("cashback") ||
        categoryName.toLowerCase().includes("hoàn tiền") ||
        note.includes("cashback") ||
        note.includes("hoàn tiền");
...
export async function getPocketBaseAccountDetails(
  sourceAccountId: string,
  ...
  const parent = account.parent_account_id
    ? allAccounts.find((item) => item.id === account.parent_account_id)
    : null;

  const statsSnapshot = await getPocketBaseAccountSpendingStatsSnapshot(
    sourceAccountId,
    new Date(),
  );
  ...
  return {
    ...
    stats: {
      ...
      missing_for_min: statsSnapshot?.minSpend
        ? Math.max(
            0,
            statsSnapshot.minSpend - (statsSnapshot.currentSpend || 0),
          )
        : null,
...
export async function loadPocketBaseTransactionsForAccount(
  sourceAccountId: string,
  limit = 2000,
...
export async function loadPocketBaseTransactions(options: {
  ...
  if (options.personId || options.personIds) {
    ...
    const filterParts = candidatePersonIds
      .map((pid) => `person_id='${escapeFilterValue(pid)}'`)
      .join(" || ");
    const records = await listAllRecords("transactions", {
      sort: "-date",
      expand:
  ...
  if (options.accountId) {
    return loadPocketBaseTransactionsForAccount(
      options.accountId,
      options.limit,
    );
  }
...
export async function getPocketBaseAccountCycleOptions(
  sourceAccountId: string,
  limit = 12,
): Promise<
  Array<{
...
export async function createPocketBaseTransaction(
  ...
  await pocketbaseRequest(`/api/collections/transactions/records`, {
    ...
    body: {
      ...
      to_account_id: data.target_account_id
        ? toPocketBaseId(data.target_account_id)
        : null,
...
export async function updatePocketBaseTransaction(
  ...
  if (data.account_id !== undefined)
    payload.account_id = toPocketBaseId(data.account_id);
  ...
  if (data.category_id !== undefined)
    payload.category_id = data.category_id
      ? toPocketBaseId(data.category_id)
      : null;
  if (data.person_id !== undefined)
    payload.person_id = data.person_id ? toPocketBaseId(data.person_id) : null;
  if (data.target_account_id !== undefined)
    payload.to_account_id = data.target_account_id
      ? toPocketBaseId(data.target_account_id)
      : null;
  if (data.shop_id !== undefined)
    payload.shop_id = data.shop_id ? toPocketBaseId(data.shop_id) : null;
  if (data.status !== undefined) payload.status = data.status;
  if (data.persisted_cycle_tag !== undefined)
    payload.persisted_cycle_tag = data.persisted_cycle_tag;
  if (data.cashback_share_percent !== undefined)
    payload.cashback_share_percent = data.cashback_share_percent;
  if (data.cashback_share_fixed !== undefined)
    payload.cashback_share_fixed = data.cashback_share_fixed;
  if (data.cashback_mode !== undefined)
    payload.cashback_mode = data.cashback_mode;
  if (
    data.metadata !== undefined ||
    data.cashback_share_percent !== undefined ||
    data.cashback_share_fixed !== undefined ||
    data.cashback_mode !== undefined ||
    data.debt_cycle_tag !== undefined ||
    data.tag !== undefined
  ) {
...
export async function voidPocketBaseTransaction(
  supabaseId: string,
): Promise<void> {
...
export async function getPocketBaseUnifiedTransactions(
  options: {
    limit?: number;
    includeVoided?: boolean;
  } = {},
  ...
  while (page <= totalPages && records.length < limit) {
    ...
    try {
      ...
      console.warn(
        `[DB:PB] transactions.unified.list: page ${page} failed, stopping pagination`,
        err,
      );
      break;
    }
