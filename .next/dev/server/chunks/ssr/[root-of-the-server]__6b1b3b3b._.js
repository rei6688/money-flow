...
async function getChecklistDataAction(bankType, year = new Date().getFullYear()) {
    try {
        ...
        if (batchError) throw batchError;
        // Fetch Funding Txns
        const fundingTxnIds = batches?.map((b)=>b.funding_transaction_id).filter(Boolean) || [];
        let fundingTxns = [];
        ...
        const enrichedBatches = batches?.map((b)=>({
                ...b,
                funding_transaction: fundingTxns.find((t)=>t.id === b.funding_transaction_id) || null
            }));
        return {
            success: true,
            data: {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__6b1b3b3b._.js.map