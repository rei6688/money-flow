...
/* __next_internal_action_entry_do_not_use__ [{"6062ab2bc1efea2d627d4c17385842356e623ff8a6":"getDashboardStats"},"",""] */ __turbopack_context__.s([
    "getDashboardStats",
    ()=>getDashboardStats
]);
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$10_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$3_react$40$19$2e$2$2e$3_$5f$react$40$19$2e$2$2e$3$2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$server$2d$reference$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/.pnpm/next@16.0.10_@babel+core@7.28.5_react-dom@19.2.3_react@19.2.3__react@19.2.3/node_modules/next/dist/build/webpack/loaders/next-flight-loader/server-reference.js [app-rsc] (ecmascript)");
...
;
;
async function getDashboardStats(month, year) {
    const defaultStats = {
        totalAssets: 0,
        monthlySpend: 0,
        monthlyIncome: 0,
    ...
    try {
        ...
        // ========================================================================
        const { data: accounts, error: assetsError } = await supabase.from('accounts').select('type, current_balance').eq('owner_id', user.id).eq('is_active', true).in('type', [
            'bank',
            'cash',
            'savings',
            'investment',
        ...
        // ========================================================================
        const { data: expenseTxns, error: spendError } = await supabase.from('transactions').select('amount, category_name:categories(name)').eq('type', 'expense').gte('occurred_at', startOfMonth.toISOString()).lte('occurred_at', endOfMonth.toISOString()).neq('status', 'void');
        ...
        const monthlySpend = expenseTxns?.reduce((sum, tx)=>sum + Math.abs(tx.amount || 0), 0) || 0;
        // ========================================================================
        // 3. SPENDING BY CATEGORY (For Chart)
        // ========================================================================
        // We need to fetch category details for the chart
        const { data: categoryStats, error: catError } = await supabase.from('transactions').select(`
        amount,
        categories (
          id, name, icon, type
        )
      `).eq('type', 'expense').gte('occurred_at', startOfMonth.toISOString()).lte('occurred_at', endOfMonth.toISOString()).neq('status', 'void');
        if (catError) throw catError;
        const categoryMap = new Map();
        categoryStats?.forEach((tx)=>{
        ...
        // 4. MONTHLY INCOME
        // ========================================================================
        const { data: incomeTxns, error: incomeError } = await supabase.from('transactions').select('amount').eq('type', 'income').gte('occurred_at', startOfMonth.toISOString()).lte('occurred_at', endOfMonth.toISOString()).neq('status', 'void');
        if (incomeError) throw incomeError;
        const monthlyIncome = incomeTxns?.reduce((sum, tx)=>sum + Math.abs(tx.amount || 0), 0) || 0;
        // ========================================================================
        // 5. TOP DEBTORS (No date filter - cumulative debt)
        // ========================================================================
        const { data: debtAccounts, error: debtError } = await supabase.from('accounts').select('id, name, owner_id, current_balance').eq('type', 'debt').gt('current_balance', 0).order('current_balance', {
            ascending: false
        }).limit(5);
        if (debtError) throw debtError;
        // Fetch profiles for debtors
        const debtorOwnerIds = debtAccounts?.map((acc)=>acc.owner_id).filter(Boolean) || [];
        const { data: profiles, error: profilesError } = await supabase.from('people').select('id, name, image_url').in('id', debtorOwnerIds);
        if (profilesError) throw profilesError;
        const profileMap = new Map(profiles?.map((p)=>[
                p.id,
                p
            ]) || []);
        const topDebtors = debtAccounts?.map((acc)=>{
        ...
        const { data: allProfiles, error: allProfilesError } = await supabase.from('people').select('id, name, image_url').eq('is_archived', false);
        if (!allProfilesError && allProfiles) {
            const profileIds = allProfiles.map((p)=>p.id);
            // Query debt transactions by person_id
            const { data: debtTxns, error: debtTxnsError } = await supabase.from('transactions').select('person_id, amount, tag, occurred_at').eq('type', 'debt').in('person_id', profileIds).neq('status', 'void').order('occurred_at', {
                ascending: false
            });
            if (!debtTxnsError && debtTxns) {
                // Also get repayments to calculate net debt
                const { data: repayTxns } = await supabase.from('transactions').select('person_id, amount').eq('type', 'repayment').in('person_id', profileIds).neq('status', 'void');
                // Calculate net repayments per person
                const repaymentByPerson = new Map();
                if (repayTxns) {
                    repayTxns.forEach((tx)=>{
                        if (tx.person_id) {
                ...
                const mapKey = (personId, tag)=>`${personId}::${tag}`;
                const cycleMap = new Map();
                debtTxns.forEach((tx)=>{
                    ...
                    if (tagValue) {
                        ...
                        label = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$date$2d$fns$40$4$2e$1$2e$0$2f$node_modules$2f$date$2d$fns$2f$format$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$locals$3e$__["format"])(parsedDate, 'MMM yy').toUpperCase();
                    }
                    const amount = Math.abs(tx.amount || 0);
                ...
                outstandingList = Array.from(cycleMap.values()).filter((item)=>item.amount > 0) // Only show actual debt
                .map((item)=>{
        ...
        const { data: refundAccount, error: refundAccountError } = await supabase.from('accounts').select('current_balance').eq('id', __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$constants$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["SYSTEM_ACCOUNTS"].PENDING_REFUNDS).single();
        ...
        const { data: refundTransactions, error: refundTxError } = await supabase.from('transactions').select('id, note, amount, occurred_at').eq('target_account_id', __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$constants$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["SYSTEM_ACCOUNTS"].PENDING_REFUNDS).neq('status', 'void').order('occurred_at', {
            ascending: false
        }).limit(3);
        if (refundTxError) {
            console.error('Error fetching refund transactions:', refundTxError);
        }
        const topRefundTransactions = refundTransactions?.map((tx)=>({
                id: tx.id,
                note: tx.note,
        ...
        let fundedBatchItemsGrouped = [];
        try {
            const { data: pendingBatchItems, error: batchError } = await supabase.from('batch_items').select('amount').eq('status', 'pending');
            if (!batchError) {
                pendingBatchCount = pendingBatchItems?.length || 0;
                pendingBatchAmount = pendingBatchItems?.reduce((sum, item)=>sum + Math.abs(item.amount || 0), 0) || 0;
            }
            // ========================================================================
            // 8b. FUNDED BATCH ITEMS (Pending confirmation, grouped by bank)
            // ========================================================================
            const { data: fundedBatches, error: fundedBatchError } = await supabase.from('batches').select('id').eq('status', 'funded');
            if (!fundedBatchError) {
                const fundedBatchIds = fundedBatches?.map((b)=>b.id) || [];
                if (fundedBatchIds.length > 0) {
                    const { data: fundedItems, error: fundedItemsError } = await supabase.from('batch_items').select('id, amount, receiver_name, note, target_account_id, accounts!batch_items_target_account_id_fkey(name)').in('batch_id', fundedBatchIds).eq('status', 'pending').order('target_account_id');
        ...
        // ========================================================================
        const { data: recentTx, error: recentTxError } = await supabase.from('transactions').select(`
        id,
        note,
        occurred_at,
        status,
        ...
        `).neq('status', 'void').order('occurred_at', {
            ascending: false
        }).limit(5);
...
(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$10_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$3_react$40$19$2e$2$2e$3_$5f$react$40$19$2e$2$2e$3$2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$server$2d$reference$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["registerServerReference"])(getDashboardStats, "6062ab2bc1efea2d627d4c17385842356e623ff8a6", null);
}),
...
/* __next_internal_action_entry_do_not_use__ [{"401bcb9549bc7d544ca4cbbf5ea020fb24877728d1":"confirmBatchItemAction"},"",""] */ __turbopack_context__.s([
...
;
;
async function confirmBatchItemAction(itemId) {
    try {
        await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$services$2f$batch$2e$service$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["confirmBatchItem"])(itemId);
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$10_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$3_react$40$19$2e$2$2e$3_$5f$react$40$19$2e$2$2e$3$2f$node_modules$2f$next$2f$cache$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["revalidatePath"])('/');
        return {
            success: true
        };
...
(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$10_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$3_react$40$19$2e$2$2e$3_$5f$react$40$19$2e$2$2e$3$2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$server$2d$reference$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["registerServerReference"])(confirmBatchItemAction, "401bcb9549bc7d544ca4cbbf5ea020fb24877728d1", null);
}),
"[project]/src/actions/refund-actions.ts [app-rsc] (ecmascript)", ((__turbopack_context__) => {
"use strict";

/* __next_internal_action_entry_do_not_use__ [{"00afcde7021f082f34dd7ae906fcee76579d2e505a":"getPendingRefundsAction","6049d23a5ff77e7f17e3ebefeadd85bdfc9b1c6291":"confirmRefundMoneyReceived"},"",""] */ __turbopack_context__.s([
...
;
;
async function confirmRefundMoneyReceived(transactionId, targetAccountId) {
    try {
        const supabase = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2f$server$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["createClient"])();
        // 1. Get the original transaction to find the refund amount
        const { data: originalTx, error: txError } = await supabase.from('transactions').select('*').eq('id', transactionId).single();
        ...
        if (originalTx.target_account_id !== __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$constants$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["SYSTEM_ACCOUNTS"].PENDING_REFUNDS) {
        ...
        const { data: newTx, error: newTxError } = await supabase.from('transactions').insert({
            ...
            created_by: userId,
            type: 'transfer',
            account_id: __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$constants$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["SYSTEM_ACCOUNTS"].PENDING_REFUNDS,
            target_account_id: targetAccountId,
            amount: -refundAmount
        }).select().single();
        ...
        // 5. Recalculate balances
        const { recalculateBalance } = await __turbopack_context__.A("[project]/src/services/account.service.ts [app-rsc] (ecmascript, async loader)");
        await recalculateBalance(__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$constants$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["SYSTEM_ACCOUNTS"].PENDING_REFUNDS);
...
;
async function getPendingRefundsAction() {
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$services$2f$transaction$2e$service$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getPendingRefunds"])();
}
;
(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$10_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$3_react$40$19$2e$2$2e$3_$5f$react$40$19$2e$2$2e$3$2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$action$2d$validate$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["ensureServerEntryExports"])([
    confirmRefundMoneyReceived,
...
(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$10_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$3_react$40$19$2e$2$2e$3_$5f$react$40$19$2e$2$2e$3$2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$server$2d$reference$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["registerServerReference"])(confirmRefundMoneyReceived, "6049d23a5ff77e7f17e3ebefeadd85bdfc9b1c6291", null);
(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$10_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$3_react$40$19$2e$2$2e$3_$5f$react$40$19$2e$2$2e$3$2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$server$2d$reference$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["registerServerReference"])(getPendingRefundsAction, "00afcde7021f082f34dd7ae906fcee76579d2e505a", null);
}),
"[project]/.next-internal/server/app/page/actions.js { ACTIONS_MODULE0 => \"[project]/src/actions/ai-reminder-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE1 => \"[project]/src/actions/ai-actions-v2.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE2 => \"[project]/src/actions/transaction-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE3 => \"[project]/src/actions/ai-learn-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE4 => \"[project]/src/services/account.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE5 => \"[project]/src/services/people.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE6 => \"[project]/src/services/shop.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE7 => \"[project]/src/services/category.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE8 => \"[project]/src/actions/bulk-transaction-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE9 => \"[project]/src/actions/log-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE10 => \"[project]/src/services/transaction.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE11 => \"[project]/src/actions/cascade-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE12 => \"[project]/src/actions/account-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE13 => \"[project]/src/services/installment.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE14 => \"[project]/src/actions/people-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE15 => \"[project]/src/actions/service-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE16 => \"[project]/src/actions/settings-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE17 => \"[project]/src/services/dashboard.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE18 => \"[project]/src/services/cashback.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE19 => \"[project]/src/actions/batch-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE20 => \"[project]/src/actions/refund-actions.ts [app-rsc] (ecmascript)\" } [app-rsc] (server actions loader, ecmascript) <locals>", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$services$2f$cashback$2e$service$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/services/cashback.service.ts [app-rsc] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$actions$2f$batch$2d$actions$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/actions/batch-actions.ts [app-rsc] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$actions$2f$refund$2d$actions$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/actions/refund-actions.ts [app-rsc] (ecmascript)");
;
;
;
;
...
"[project]/.next-internal/server/app/page/actions.js { ACTIONS_MODULE0 => \"[project]/src/actions/ai-reminder-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE1 => \"[project]/src/actions/ai-actions-v2.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE2 => \"[project]/src/actions/transaction-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE3 => \"[project]/src/actions/ai-learn-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE4 => \"[project]/src/services/account.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE5 => \"[project]/src/services/people.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE6 => \"[project]/src/services/shop.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE7 => \"[project]/src/services/category.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE8 => \"[project]/src/actions/bulk-transaction-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE9 => \"[project]/src/actions/log-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE10 => \"[project]/src/services/transaction.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE11 => \"[project]/src/actions/cascade-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE12 => \"[project]/src/actions/account-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE13 => \"[project]/src/services/installment.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE14 => \"[project]/src/actions/people-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE15 => \"[project]/src/actions/service-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE16 => \"[project]/src/actions/settings-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE17 => \"[project]/src/services/dashboard.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE18 => \"[project]/src/services/cashback.service.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE19 => \"[project]/src/actions/batch-actions.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE20 => \"[project]/src/actions/refund-actions.ts [app-rsc] (ecmascript)\" } [app-rsc] (server actions loader, ecmascript)", ((__turbopack_context__) => {
"use strict";
...
__turbopack_context__.s([
    ...
    "401bcb9549bc7d544ca4cbbf5ea020fb24877728d1",
    ()=>__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$actions$2f$batch$2d$actions$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["confirmBatchItemAction"],
    "40234e07371f4dc2e219b63863d670b919e5da58b8",
    ...
    "6049d23a5ff77e7f17e3ebefeadd85bdfc9b1c6291",
    ()=>__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$actions$2f$refund$2d$actions$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["confirmRefundMoneyReceived"],
    ...
    "6062ab2bc1efea2d627d4c17385842356e623ff8a6",
    ()=>__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$services$2f$dashboard$2e$service$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getDashboardStats"],
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c50010b8._.js.map